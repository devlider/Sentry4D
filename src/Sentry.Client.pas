...
type
  ...
  private
    ...
    function BuildEventPayload(const EventID: string; const E: Exception): string;
...
begin
  ...
  begin
    ...
    vData.AddPair('arguments', vArguments.ToJSON);
  end;
...
function TSentryClient.BuildEventPayload(const EventID: string; const E: Exception): string;
var
  Obj, Exo, Exs, TagsObj, ctxt, bread: TJSONObject;
  Arr: TJSONArray;
  k: string;
begin
...
procedure TSentryClient.CaptureException(const E: Exception);
var
...
begin
  ID := GeraUUID(True);
  Payload := BuildEventPayload(ID, E);
